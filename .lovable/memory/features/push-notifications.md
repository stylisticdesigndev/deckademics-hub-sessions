...
- Student→instructor automated alerts (running late, mark absent, undo absence — RunningLateButton, UpcomingClassCard, useStudentClassAttendance) fan out to ALL of a student's instructors (primary + secondary) via getStudentInstructorIds(studentId) in notifyPush.ts, which reads student_instructors (role primary/secondary) and falls back to students.instructor_id. Each instructor gets the in-app message + push. Direct thread replies (StudentMessages) stay one-to-one to the chosen instructor.
